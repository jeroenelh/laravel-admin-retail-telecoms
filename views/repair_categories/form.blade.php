            ...
            <div class="box">
                <div class="box-body">
                    <div class="col-md-4 form-group">
                        <label>@lang("laravel-admin-retail-telecoms::repair_category.display_title")</label>
                        {{ Form::text('display_title', $data['model']->display_title, ['class' => 'form-control']) }}
                    </div>
                    <div class="col-md-4 form-group">
                        <label>@lang("laravel-admin-retail-telecoms::repair_category.slug")</label>
                        {{ Form::text('slug', $data['model']->slug, ['class' => 'form-control']) }}
                    </div>
                    <div class="col-md-4 form-group">
                        <label>@lang("laravel-admin-retail-telecoms::repair_category.media")</label>
                        {{ Form::file('file', ['class' => 'form-control']) }}
                    </div>
                    <div class="col-md-8 form-group">
                        <label>@lang("laravel-admin-retail-telecoms::repair_category.description")</label>
                        {{ Form::textarea('description', $data['model']->description, ['class' => 'form-control']) }}
                    </div>
                    @if($data['model']->medium)
                        <div class="col-md-4">
                            <label>@lang("laravel-admin-retail-telecoms::repair_category.media")</label><br />
                            <img src="{{ '/images/uploads/repair/'.$data['model']->medium->storage_name }}" />
                        </div>
                    @endif
                </div>
            </div>
