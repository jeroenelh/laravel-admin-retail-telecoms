            ...
            <div class="box">
                <div class="box-body">
                    <div class="col-md-4 form-group">
                        <label>@lang("laravel-admin-retail-telecoms::brand.display_title")</label>
                        {{ Form::text('display_title', $data['model']->display_title, ['class' => 'form-control']) }}
                    </div>
                    <div class="col-md-4 form-group">
                        <label>@lang("laravel-admin-retail-telecoms::brand.slug")</label>
                        {{ Form::text('slug', $data['model']->slug, ['class' => 'form-control']) }}
                    </div>
                    <div class="col-md-4 form-group">
                        <label>@lang("laravel-admin-retail-telecoms::brand.media")</label>
                        {{ Form::file('file', ['class' => 'form-control']) }}
                    </div>
                    @if($data['model']->medium)
                        <div class="col-md-6">
                            <label>@lang("laravel-admin-retail-telecoms::accessory.media")</label><br />
                            <img src="{{ '/images/uploads/brand/'.$data['model']->medium->storage_name }}" />
                        </div>
                    @endif
                </div>
            </div>
